const { Conversation, User, Message, Friend, Profile } = require("../models");
    ...
    try {
      ...
      let conversationsWithLastMessage = conversations.map((conversation) => {
        ...
        if (!senderLastMessage && !receiverLastMessage) {
          ...
          lastMessage =
            senderLastMessage.timestamp >
            receiverLastMessage.timestamp.getTime()
              ? senderLastMessage
              : receiverLastMessage;
  ...
  static async createConversation(req, res, next) {
    try {
      ...
      if (conversation) {
        res
          .status(400)
          .json({ message: `You've added to the conversation with him` });
      }
